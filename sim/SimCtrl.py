        ...
        for nd_src in sim_core.nodes:
            ...
            for nd_dst in sim_core.nodes:
                if(nd_src == nd_dst):
                    ...
                    path_spf = self.path_db[(nd_src, nd_dst)]
                    distance = len(path_spf[0]) - 1
                    
                    n1 = sim_core.nodeobjs[nd_src].n_hosts
                    n2 = sim_core.nodeobjs[nd_dst].n_hosts
                    anti_gravity_weight = float(distance * distance )/ (n1 * n2)
                    value_list.append((nd_dst, anti_gravity_weight))
            anti_gravity_table[nd_src] = value_list
        return anti_gravity_table

    def build_gravity_weight(self, sim_core):
        """Build a gravity weight matrix.
        ...
        for nd_src in sim_core.nodes:
            ...
            for nd_dst in sim_core.nodes:
                if(nd_src == nd_dst):
                    ...
                    n1 = sim_core.nodeobjs[nd_src].n_hosts
                    n2 = sim_core.nodeobjs[nd_dst].n_hosts
                    gravity_weight = n1 * n2 / (distance * distance)
