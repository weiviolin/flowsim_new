...
class SimFlowGen:
	...
	def pick_dst(self, src_ip, sim_core):
		...
		while True:
			...
			if (cfg.FLOWGEN_SRCDST_MODEL == 'gravity'):
				dst_ip = self.pick_dst_gravity(src_ip, sim_core, 'gravity') 
			if (cfg.FLOWGEN_SRCDST_MODEL == 'anti_gravity'):
				dst_ip = self.pick_dst_gravity(src_ip, sim_core, 'anti_gravity')  # Default to 'uniform'
	...
	def pick_dst_gravity(self, src_ip, sim_core, is_anti):
		"""Given src_ip, pick a dst_ip using random model based on gravity weight.

		Args:
			src_ip (netaddr.IPAddress)
			sim_core (instance of SimCore)
			is_anti (determine gravity or anti_gravity)

		Extra Notes:
			grabity: hosts with large weight have more probability to be select as destination node.
			anti_gravity: hosts with small weight have more probability to be select as destination node.

		"""
		dst_ip = 0
		if(is_anti == 'gravity'):
			gravity_table = sim_core.ctrl.gravity_table
		if(is_anti == 'anti_gravity'):
			gravity_table = sim_core.ctrl.anti_gravity_table
		...
		for node in sim_core.nodes:
			if(sim_core.nodeobjs[node].end_ip >= src_ip and sim_core.nodeobjs[node].base_ip <= src_ip ):
				designed_src_node = node
				break
			
		value_list = gravity_table[designed_src_node]
		total_weight = 0
		...
		random_number = rd.uniform(0, total_weight)
		for tp in value_list:
			if random_number <= tp[1]:
				...
				random_number -= tp[1]
		dst_ip = sim_core.nodeobjs[designed_dst_node].base_ip + rd.randint(0, sim_core.nodeobjs[designed_dst_node].n_hosts - 1)
